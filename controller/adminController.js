...
export const AddAdminBookingController = async (req, res) => {
  try {
    const {
      ...
      OnBoardDate,
      dutyHr,
      addHistory,time,appoinment,prescription,images,date,hospitalId
    } = req.body;
...
if(updatedOrder.type === 4444 && updatedOrder.status == 1){
...
if(updatedOrder.type === 444 && updatedOrder.status == 1){
...
export const getAllUserAdmin = async (req, res) => {
 ...
 if (userId) {
   query.headId = new mongoose.Types.ObjectId(userId);
}
...
export const AdminGetAllEmployee_last = async (req, res) => { 
  try {
    // Extract the category (which can be an array of ObjectIds), type, and coordinates from query parameters
    const { type,filter,phone,empType,department } = req.query;
  ...
  try {
   ...
   fillter.headId = new mongoose.Types.ObjectId(headId);
    ...
    if (department && department !== "none") {
      let departmentIdsRaw = [];

      // If frontend sends: ?department=665f8b1c...,665f8b2d...
      if (Array.isArray(department)) {
        departmentIdsRaw = department;
      } else if (typeof department === "string") {
        const trimmed = department.trim();

        // If frontend sends JSON string: '["id1","id2"]'
        if (trimmed.startsWith("[") && trimmed.endsWith("]")) {
          try {
            const parsed = JSON.parse(trimmed);
            if (Array.isArray(parsed)) {
              departmentIdsRaw = parsed;
            }
          } catch (e) {
            console.log("Invalid JSON department param:", department);
          }
        } else {
          // comma-separated
          departmentIdsRaw = trimmed.split(",");
        }
      }

      // Clean + validate ObjectIds
      const departmentIds = departmentIdsRaw
        .map((id) => String(id).trim())
        .filter((id) => mongoose.Types.ObjectId.isValid(id));

      console.log("departmentIdsRaw:", departmentIdsRaw);
      console.log("valid departmentIds:", departmentIds);

      if (departmentIds.length > 0) {
        // Mongoose can auto-cast strings to ObjectId here
        fillter.department = { $in: departmentIds };
      }
      // If no valid ids -> we just skip department filter (no crash)
    }
    // ---------- END DEPARTMENT FIX ----------
    ...
    // If department param was present, return unique subDepartments as well
    let uniqueSubDepartments = [];
    if (department && department !== "none") {
      const subDeptSet = new Set();

      users.forEach((u) => {
        if (Array.isArray(u.subDepartments)) {
          u.subDepartments.forEach((sd) => {
            if (sd !== null && sd !== undefined && sd !== "") {
              subDeptSet.add(String(sd));
            }
          });
        }
      });

      uniqueSubDepartments = [...subDeptSet];
    }
    ...
    return res.status(200).send({
      ...
      success: true,
      Users: users,
      subDepartments: uniqueSubDepartments, // will be [] if no department filter
    });
  } catch (error) {
